...
class TestUserLogin:
    ...
    def test_login_existing_user_success(self, registered_user):
        login_data = {
            Fields.EMAIL: registered_user[Fields.EMAIL],
            Fields.PASSWORD: registered_user[Fields.PASSWORD]
        }
        ...
        with allure.step("Проверка тела ответа"):
            ...
            assert response_data["user"][Fields.EMAIL] == registered_user[Fields.EMAIL]
            assert response_data["user"][Fields.NAME] == registered_user[Fields.NAME]
            assert "accessToken" in response_data

    @allure.title("Вход с неверным логином или паролем")
    @pytest.mark.parametrize("wrong_field", TestData.LOGIN_FIELDS_TO_TEST)
    def test_login_wrong_credentials_fail(self, registered_user, wrong_field):
        login_data = {
            Fields.EMAIL: registered_user[Fields.EMAIL],
            Fields.PASSWORD: registered_user[Fields.PASSWORD]
        }
